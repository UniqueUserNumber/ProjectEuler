...
further by
qanalysis even though it is known that the greatest number that cannot
...
for x in range(1, Limit):
    for y in range(x * 2, Limit, x):
        divisorsum[y] += x
Numbers = [x for (x, z) in enumerate(divisorsum) if z > x]

Listings = [False] * Limit
for x in Numbers:
    for y in Numbers:
        if x + y < Limit:
            Listings[x + y] = True
...
ans = sum(i for (i, x) in enumerate(Listings) if not x)
